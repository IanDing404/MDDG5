...
rm -f ${log}
  ...
  #conv_SimSET_projdata_to_STIR_modified.sh ${weight_filename} ${num_seg} "${SCANNER}" 2>&1 > ${log}
#fi
if [ ! -z "${weight_filename}" ]; then
  conv_SimSET_projdata_to_STIR_modified_orders.sh ${weight_filename} ${num_seg} "${SCANNER}" 2>&1 > ${log}
fi
